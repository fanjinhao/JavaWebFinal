package com.fayne.service.impl;
...
import com.fayne.dao.BaseDAO;
import com.fayne.entity.PageBean;
import com.fayne.entity.Topic;
import com.fayne.service.TopicService;
...
public class TopicServiceImpl implements TopicService {
	...
	public Long getNoReplyTopicCount(Topic s_topic) {
		List<Object> param = new LinkedList<Object>();
		// StringBuffer hql=new StringBuffer("select count(*) from Topic t where
		// t.id not in (select t.id from Reply r,Topic t where
		// r.topic.id=t.id)");
		StringBuffer hql = new StringBuffer(
				"select count(*) from Topic where id not in (select r.topic.id from Reply r)");
		if (s_topic != null) {
			if (s_topic.getSection().getId() > 0) {
				hql.append(" and sectionId=?");
				param.add(s_topic.getSection().getId());
